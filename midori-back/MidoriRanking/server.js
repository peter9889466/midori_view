...
const getEcoData = async () => {
  ...
  for (const url of urls) {
    try {
      console.log(`📄 ${url}에서 ranking.json 시도 중...`);
      const response = await fetch(url);
      if (response.ok) {
        console.log(`✅ ${url}에서 ranking.json 로드 성공`);
  ...
  console.error('❌ 모든 URL에서 ranking.json 로드 실패');
  return null;
};
  ...
  try {
    ...
    if (existingRows.length > 0) {
      ...
      const formattedData = existingRows.map(row => ({
        RANK_ID: row.RANK_ID,
        RANK_COUNTRY: row.RANK_COUNTRY,
        RANK_PRODUCT: row.RANK_PRODUCT,
        RANK_CATEGORY: row.RANK_CATEGORY,
        RANK_COUNTRY_CODE: row.RANK_COUNTRY_CODE || '',
        EXPORT_VALUE: row.EXPORT_VALUE || 0,
        IMPORT_VALUE: row.IMPORT_VALUE || 0,
        RANK_PERIOD: row.RANK_PERIOD,
        HS_CODE: row.HS_CODE || ''
      }));
    ...
    if (results.length > 0) {
      ...
      const insertSQL = `
        INSERT INTO RANKING_TABLE (
          ...
          IMPORT_VALUE, RANK_PERIOD, RANK_COUNTRY_CODE, HS_CODE, CREATED_AT
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
      ...
      for (const item of results) {
        await connection.execute(insertSQL, [
          ...
          item.country_code, item.hs_code
        ]);
      }
    ...
    // 저장된 데이터를 프론트엔드 형식으로 변환
    const formattedResults = results.map(item => ({
      RANK_ID: item.id,
      RANK_COUNTRY: item.country,
      RANK_PRODUCT: item.product,
      RANK_CATEGORY: item.category,
      RANK_COUNTRY_CODE: item.country_code,
      EXPORT_VALUE: item.export_value,
      IMPORT_VALUE: item.import_value,
      RANK_PERIOD: item.RANK_PERIOD,
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      const insertSQL = `
        INSERT INTO RANKING_TABLE (
          RANK_ID, RANK_COUNTRY, RANK_PRODUCT, RANK_CATEGORY, EXPORT_VALUE, 
          IMPORT_VALUE, RANK_PERIOD, RANK_COUNTRY_CODE, HS_CODE
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
          EXPORT_VALUE = VALUES(EXPORT_VALUE),
          IMPORT_VALUE = VALUES(IMPORT_VALUE)
...
const startServer = async () => {
  ...
  app.listen(PORT, () => {
    ...
    console.log('🔧 CORS 문제 해결: 여러 포트 지원');
    console.log('🎉==================================🎉\n');
  });
};
