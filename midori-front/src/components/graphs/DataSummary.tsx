...
export default function DataSummary({
    ...
    return (
        <div className="w-1/4 min-w-0 flex-shrink-0">
            <div className="bg-white rounded-lg p-4 border border-gray-200 h-[400px] flex flex-col overflow-hidden">
                <div className="flex flex-col space-y-2 h-full">
                    <span className="text-lg font-bold text-[#6AAE4A] truncate" title={displayProductName}>{displayProductName}</span>
                    <span className="text-sm font-bold truncate">총 수출입액</span>
                    <span className="text-4xl font-bold truncate">${formatNumber(totalTradeAmount)}</span>
                    <div className="text-sm space-y-1">
                        <div className="truncate">수출액 ${formatNumber(apiExportTotal)}</div>
                        <div className="truncate">수입액 ${formatNumber(apiImportTotal)}</div>
                        <div className="truncate">평균액 ${formatNumber(totalTradeAmount / (dataLength || 1))}</div>
                        <div className="truncate">그래프 기간: {기간표현}</div>
                    </div>
                    <div className="text-sm text-gray-500 mt-2 flex-1 overflow-hidden break-words" style={{wordBreak: 'break-word'}}>
                        {productDescription}
                    </div>
                    <div className="mt-auto">
                        ...
                        <button 
                            ...
                            className="mt-2 px-4 py-2 bg-blue-500 text-white rounded text-xs truncate w-full"
                        >
                            차트 테스트
                        </button>
                        {isGeneratingPDF && (
                            <div className="text-xs text-green-600 mt-1 flex items-center gap-1">
                                ...
                                <span className="truncate">PDF 생성 중...</span>
                            </div>
                        )}
                    </div>
                </div>
